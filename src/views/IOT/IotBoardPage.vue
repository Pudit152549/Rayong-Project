...
onMounted(async () => {
  await tasksStore.fetchByBoardSlug("iot");
});

const rows = computed(() => tasksStore.byBoard["iot"] ?? []);
...
const filteredRows = computed(() => {
  ...
  return (tasksStore.byBoard["iot"] ?? []).filter((r) => {
    const matchKw =
      !kw ||
    ...
    h(
      ...
      onClick: () =>
        router.push({
          name: "Edit",
          params: { id: row.id },
          query: { from: "IotBoard", deptKey: "iot" }
        })
      },
...
const confirmDelete = (row: RowData) => {
  dialog.error({
    ...
    onPositiveClick: async () => {
      ...
      // ตัวอย่าง:
      await tasksStore.deleteTask("iot", row.id);
...
const addData = () => {
  router.push({ name: "AddData", query: { from: "IotBoard", deptKey: "iot" } });
};
</script>
...
<style scoped>
.gradient-text {
  background: linear-gradient(90deg, #027e00, #7cff8b, #39841e);
